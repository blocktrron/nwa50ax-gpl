...
ifeq ($(DUMP),1)
  ...
  ifeq ($(TARGET_BUILD),1)
    SUBTARGET_EXIST:=$(shell ls ./$(CONFIG_TARGET_SUBTARGET)/patches-$(KERNEL_PATCHVER) 2>/dev/null)
    ifdef SUBTARGET_EXIST
        MODELPATCH_EXIST:=$(shell ls ./$(CONFIG_TARGET_SUBTARGET)/$(CONFIG_ZYUMAC_MODEL_NAME)/patches-$(KERNEL_PATCHVER) 2>/dev/null)
        ifdef MODELPATCH_EXIST
            $(shell cp -fpR ./$(CONFIG_TARGET_SUBTARGET)/$(CONFIG_ZYUMAC_MODEL_NAME)/patches-$(KERNEL_PATCHVER)/* ./$(CONFIG_TARGET_SUBTARGET)/patches-$(KERNEL_PATCHVER)/)
        endif
        PATCH_DIR ?= ./$(CONFIG_TARGET_SUBTARGET)/patches-$(KERNEL_PATCHVER)
    else 
