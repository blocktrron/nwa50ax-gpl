...
def init_drivers_from_tarball(driver,pkgsrc):
	...
	defconfig_dest = os.path.join(topdir, ".config")
	zyUMAC_ver = ""
	zyUMAC_model = ""
	for line in open(str(defconfig_dest), 'r'):
		if line.startswith('CONFIG_KERNEL_ZYUMAC_VER'):
			zyUMAC_ver = line.strip().split('=')[1].strip('"')
			print ("install zyUMAC patch !!")
			zyUMAC_patch_src = os.path.join(topdir, "nucleus/feeds/kernel/zyUMAC/src/MTK{0}/patches".format(str(zyUMAC_ver)))
			zyUMAC_patch_dest = os.path.join(topdir, "package/mtk/drivers/mt_wifi/patches")
			if not os.listdir(str(zyUMAC_patch_src)):
				break
			if not os.path.exists(str(zyUMAC_patch_dest)):
				os.makedirs(str(zyUMAC_patch_dest))
			cmd = """cp -rf {patch_src}/* {patch_dest}""".format(
				patch_src=str(zyUMAC_patch_src), patch_dest=str(zyUMAC_patch_dest))
			execute(cmd)
			print ("copy zyUMAC source !!")
			cmd = """mkdir {linuxdir}/drivers/net/wireless/mediatek/zyUMAC""".format(linuxdir=linuxdir)
			execute(cmd)
			cmd = """cp -rf {topdir}/nucleus/feeds/kernel/zyUMAC/src/* {linuxdir}/drivers/net/wireless/mediatek/zyUMAC""".format(
					topdir=topdir, linuxdir=linuxdir)
			execute(cmd)

			print("copy MTK wifi fw patch !!")
			fw_patches_src = os.path.join(topdir, "nucleus/feeds/kernel/zyUMAC/src/MTK{0}/fw-patches".format(str(zyUMAC_ver)))
			if not os.listdir(str(fw_patches_src)):
				print("{0} not exists!".format(str(fw_patches_src)))
			cmd = """cp -rf {patch_src}/* {linuxdir}/drivers/net/wireless/mediatek/""".format(patch_src=str(fw_patches_src), linuxdir=linuxdir)
			execute(cmd)

		if line.startswith('CONFIG_ZYUMAC_MODEL_NAME'):
			zyUMAC_model = line.strip().split('=')[1].strip('"')

		if zyUMAC_ver != "" and zyUMAC_model != "":
			zyUMAC_txpwr_src = os.path.join(topdir, "nucleus/feeds/kernel/zyUMAC/src/MTK{ver}/txpwr/{model}".format(ver=str(zyUMAC_ver), model=str(zyUMAC_model)))
			zyUMAC_txpwr_dest = os.path.join(topdir, "{linuxdir}/drivers/net/wireless/mediatek/mt_wifi/txpwr/sku_tables".format(linuxdir=linuxdir))
			if not os.listdir(str(zyUMAC_txpwr_src)):
				print("{0} not exists!".format(str(zyUMAC_txpwr_src)))
				break
			if not os.path.exists(str(zyUMAC_txpwr_dest)):
				print("{0} not exists!".format(str(zyUMAC_txpwr_dest)))
				break
			print ("install zyUMAC MTK{ver} {model} txpower table !!".format(ver=str(zyUMAC_ver), model=str(zyUMAC_model)))
			cmd = """cp -rf {txpwr_src}/* {txpwr_dest}""".format(
				txpwr_src=str(zyUMAC_txpwr_src), txpwr_dest=str(zyUMAC_txpwr_dest))
			execute(cmd)
			cmd = """rename -f 's/\.csv/\.dat/' {txpwr_dest}/*.csv""".format(txpwr_dest=str(zyUMAC_txpwr_dest))
			execute(cmd)
			cmd = """dos2unix {txpwr_dest}/*""".format(txpwr_dest=str(zyUMAC_txpwr_dest))
			execute(cmd)
			cmd = """make -C {linuxdir}/drivers/net/wireless/mediatek/mt_wifi/embedded build_power_limit_tables""".format(linuxdir=linuxdir)
			execute(cmd)
			break
