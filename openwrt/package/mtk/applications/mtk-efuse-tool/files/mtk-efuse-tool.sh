...
MTK_SMC_DIR="/sys/kernel/debug/mtk-smc"
...
usage()
{
	echo "Usage: mtk-efuse-tool.sh [OPTION] [HASH_NUMBER] [PKEY_HASH_FILE]"
	echo " OPTION are:"
	...
	echo "    ea       enable anti-rollback of BROM"
	echo "    ca       check if enable anti-rollback of BROM"
	echo "    ua       update eFuse anti-rollback version"
	echo " Example:"
	echo "  @For secure boot:"
	echo "    mtk-efuse-tool.sh wh 0 bl2.img.signkeyhash"
	echo "    mtk-efuse-tool.sh rh 0"
	echo "    mtk-efuse-tool.sh lh 0"
	echo "    mtk-efuse-tool.sh es"
	echo "  @For anti-rollback:"
	echo "    mtk-efuse-tool.sh ea"
	echo "    mtk-efuse-tool.sh ua"
	exit 1
}
...
if [ "${OPTION}" != "wh" -a "${OPTION}" != "rh" -a \
     ...
     "${OPTION}" != "dj" -a "${OPTION}" != "cj" -a \
     "${OPTION}" != "ea" -a "${OPTION}" != "ca" -a \
     "${OPTION}" != "ua" ]; then
...
if [ ! -d ${MTK_SMC_DIR} ]; then
	echo -e "${RED_FONT_YELLOW_BKG_ST}"\
		...
		"${RED_FONT_YELLOW_BKG_RST}"
	echo "please install mtk-smc module first"
	exit 1
fi

get_public_key_hash_from_file()
{
	PKEY_HASH1=$(hexdump -e '/4 "%08X\n"' -s 0  -n 4 ${PKEY_HASH_FILE})
	PKEY_HASH2=$(hexdump -e '/4 "%08X\n"' -s 4  -n 4 ${PKEY_HASH_FILE})
	PKEY_HASH3=$(hexdump -e '/4 "%08X\n"' -s 8  -n 4 ${PKEY_HASH_FILE})
	PKEY_HASH4=$(hexdump -e '/4 "%08X\n"' -s 12 -n 4 ${PKEY_HASH_FILE})
	PKEY_HASH5=$(hexdump -e '/4 "%08X\n"' -s 16 -n 4 ${PKEY_HASH_FILE})
	PKEY_HASH6=$(hexdump -e '/4 "%08X\n"' -s 20 -n 4 ${PKEY_HASH_FILE})
	PKEY_HASH7=$(hexdump -e '/4 "%08X\n"' -s 24 -n 4 ${PKEY_HASH_FILE})
	PKEY_HASH8=$(hexdump -e '/4 "%08X\n"' -s 28 -n 4 ${PKEY_HASH_FILE})
...
get_smc_results()
{
	...
	R0_HEX=$(printf "%016X" ${R0})
	R1_HEX=$(printf "%016X" ${R1})
	R2_HEX=$(printf "%016X" ${R2})
	R3_HEX=$(printf "%016X" ${R3})
}
...
if [ "${OPTION}" == "ea" ]; then
        ...
        get_smc_results
        if [ ${R0} -eq 0 ] 2>/dev/null; then
                echo "Enable Anti-Rollback SUCCESS"
        else
                echo "Enable Anti-Rollback FAIL"
                echo "R0 = 0x${R0_HEX}"
        fi
fi
...
if [ "${OPTION}" == "ca" ]; then
        ...
        get_smc_results
        if [ ${R0} -eq 1 ] 2>/dev/null; then
                echo "Anti-Rollback is enabled"
        else
                echo "Anti-Rollback is disabled"
        fi
fi

if [ "${OPTION}" == "ua" ]; then
        SMC_FUNCTION_ID=C2000521

        echo -e "${RED_FONT_YELLOW_BKG_ST}"\
                "eFuse Anti-Rollback Version will be updated."\
                "${RED_FONT_YELLOW_BKG_RST}"
        read -p "Continue? <y/n>" prompt
        [ ${prompt} != "y" ] && exit 1
        ...
        get_smc_results
        if [ ${R0} -eq 0 ] 2>/dev/null; then
                echo "Update eFuse Anti-Rollback Version SUCCESS"
        else
                echo "Update eFuse Anti-Rollback Version FAIL"
                echo "R0 = 0x${R0_HEX}"
        fi
fi
